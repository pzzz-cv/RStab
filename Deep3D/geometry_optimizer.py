...
class GeometryOptimizer:
    ...
    def __init__(self, opt):
        ...
        # self.copyfolder(self.opt.video_path, os.path.join(self.output_dir,'images'))

        self.seq_io = SequenceIO(opt, preprocess=True)
    ...
    def copyfolder(self, src, des):
        ...
        for filename in pathDir: #遍历pathDir下的所有文件filename
            from_path = os.path.join(src, filename) #旧文件的绝对路径(包含文件的后缀名)
            to_path = os.path.join(des, filename )        #新文件的绝对路径

            if not os.path.isdir(des):# 如果 to_path 目录不存在，则创建
                os.makedirs(des)
    ...
    def save_results(self, items, start_idx, end):
        # depth
        save_indices = range(start_idx, end)
