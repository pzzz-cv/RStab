...
print(os.path.join(video_dir, 'images/*.png'))
# pwc_model_fn = './pwc_net.pth.tar';
net = PwcNet()
net.load_state_dict(torch.load('./trained_models/sintel.pytorch'))
net.cuda()
net.eval()
...
while batch_end < len(frame_names):
    batch_end = min(len(frame_names), batch_begin + 7)
    frames = [imageio.imread(frame_names[j]) for j in range(batch_begin, batch_end)]
