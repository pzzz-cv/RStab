...
def compute_warp_maps(seq_io, warper, compensate_poses, post_process=False):
    ...
    while batch_begin < len(seq_io):
        ...
        segment = range(batch_begin, batch_end)
...
def run(opt):
    ...
    warper = Warper(opt, seq_io.get_intrinsic(seq_io.need_resize)).to(device)
    ...
    crop_t, crop_b, crop_l, crop_r = 0, seq_io.origin_height, 0, seq_io.origin_width
    ...
    while batch_begin < len(seq_io):
        ...
        seq_io.write_images(reproj_imgs[..., crop_t:crop_b, crop_l:crop_r], batch_begin)#[..., crop_t:crop_b, crop_l:crop_r]
        batch_begin = batch_end
