...
def convert_scene_output_to_glb(outdir, imgs, pts3d, mask, focals, cams2world, cam_size=0.05, show_cam=True,
    ...
    # Define the dtype for the structured array
        ...
        ply_data.write(path)
    output_path = "/data4/pengzhan/CODE/monst3r/output/Davis/%s/ply"%(os.path.basename(outdir))
    os.makedirs(output_path,exist_ok=True)
    for i in range(len(pts3d)):
        storePly(os.path.join(output_path,"%03d.ply"%(i)),pts3d[i].reshape(-1, 3), imgs[i].reshape(-1, 3))
    # import pdb
    # pdb.set_trace()
