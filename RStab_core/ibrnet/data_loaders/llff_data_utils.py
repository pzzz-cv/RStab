...
def _minify_v0(basedir, factors=[], resolutions=[]):
    needtoload = False
    for r in factors:
    ...
    for r in factors + resolutions:
        ...
        for i in range(imgs_down.shape[0]):
            imageio.imwrite(os.path.join(imgdir, 'image{:05d}.png'.format(i)), (255*imgs_down[i]).astype(np.uint8))
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    if factor is not None and factor != 1:
        sfx = '_{}'.format(factor)
        _minify_v0(basedir, factors=[factor])
        factor = factor
    elif height is not None:
        factor = sh[0] / float(height)
        width = int(sh[1] / factor)
        _minify_v0(basedir, resolutions=[[height, width]])
    ...
    disp = np.stack(disp, 0)  

    return poses, disp, bds, imgs, imgfiles
...
def load_llff_data(basedir, height=None, factor=None, recenter=True, bd_factor=.75,
                   spherify=False, path_zflat=False, load_imgs=True):
    out = _load_data(basedir, factor=factor, load_imgs=load_imgs, height=height)  # factor=8 downsamples original imgs by 8x
    if out is None:
        return
    else:
        poses, disp, bds, imgs, imgfiles = out
    ...
    return images, poses, depth, bds, render_poses, i_test, imgfiles




if __name__ == '__main__':
    for scene in ['Balloon1', 'Balloon2', 'dynamicFace', 'Jumping', 'Playground', 'Skating', 'Truck', 'Umbrella']:
    #flows_fwd, flows_bwd = [], []
        basedir  = '/data4/pengzhan/DATASET/nvidia/{}/dense'.format(scene)
        # images, poses, bds, render_poses, i_test, img_files = load_llff_data(scene_path)
        poses, bds, imgs, imgfiles = _load_data(basedir,height=360, width=640)
        # print(bds)
