...
def render_single_image(ray_sampler,
    ...
    N_rays = ray_batch['rays'].shape[0]


    for i in range(0, N_rays, chunk_size):
        chunk = OrderedDict()
        for k in ray_batch:
            if k in ['camera', 'depth_range', 'src_rgbs', 'src_cameras', 'raw_depth', 'src_flows', 'num_view']:#与光线无关，不需要修改
                chunk[k] = ray_batch[k]
            elif ray_batch[k] is not None:#与光线相关
