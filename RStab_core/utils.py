...
def get_vertical_colorbar(h, vmin, vmax, cmap_name='inferno_r', label=None, cbar_precision=2):
    '''
    :param w: pixels
    :param h: pixels
...
def colorize_np(x, cmap_name='inferno_r', mask=None, range=None, append_cbar=False, cbar_in_image=False, cbar_precision=2):
    '''
...
# tensor
def colorize(x, cmap_name='inferno_r', mask=None, range=None, append_cbar=False, cbar_in_image=False):
    device = x.device
    x = x.cpu().numpy()
    if mask is not None:
