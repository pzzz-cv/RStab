...
class SequenceIO(data.Dataset):
    def __init__(self, opt, preprocess=True): 
        ...
        # inter = os.listdir(self.root+'/flows/')
    ...
    def load_video(self): 
        ...
        self.image_names = sorted(list(glob.glob(self.input_video+'/*.png')))
    ...
    def factor_intrinsic(self, factor):
        self.intrinsic_res = self.intrinsic.clone()
        self.intrinsic_res[0] *= (1.0 / factor)
        self.intrinsic_res[1] *= (1.0 / factor)
    ...
    def generate_flows(self):
        ...
        for i in self.opt.intervals:
            ...
            ret = os.system('python video_raft.py {} {}/{} {}'.format(self.input_video, self.opt.output_dir, self.opt.name, i))
            assert ret == 0, "Failed to run RAFT, please check the environment of RAFT"
        os.chdir('..')
